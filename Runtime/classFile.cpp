...
ClassFile::ClassFile(string classFileName, ClassHeap* pClassHeap, ObjectHeap* objectHeap) : classHeap(pClassHeap)
{
	...
	{
		cout << "File (" << classFileName << ") load failed!" << endl;
		throw 25;
	}
	magic = getu4(p); p += 4;
...
int ClassFile::getAttrName(u2 attr_name_index, string &attr_value)
{
	u1 * pN;
...
method_info_w_code ClassFile::getMethod(string methodName, string methodDescription)
{
	for (int i = 0; i < methods_count; i++)
	{
...
ClassFile * ClassFile::setClassByMethod(string p_methodName, string p_methodDescription)
{
	for (int i = 0; i < methods_count; i++)
	{
		string name, description;
...
int ClassFile::getFieldIndex(string fieldName)
{
	int fieldIndex = -1;
	if(super_class != 0)
	{
